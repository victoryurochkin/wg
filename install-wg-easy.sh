...
inst_write_configs() {
    ...
    # Хэш ОБЯЗАТЕЛЬНО экранируем: каждый $ заменяем на $$.
    # Проверено на практике: Docker Compose пропускает содержимое файла,
    # подключённого через env_file:, через тот же интерполятор, что и сам
    # compose.yml. Хвост bcrypt-хэша после второго $ (например,
    # "aChFyLkrw...") выглядит как валидное имя переменной
    # ([_a-zA-Z][_a-zA-Z0-9]*), и без экранирования Compose тихо
    # подставляет туда пустую строку с предупреждением вида:
    #   WARN The "<хвост хэша>" variable is not set. Defaulting to a blank string.
    # — в контейнер уходит обрезанный хэш, вход в панель не работает.
    # Проверяй логин на https://${WG_DOMAIN} после установки — если снова
    # увидишь такой WARN в docker compose logs, значит где-то по пути
    # экранирование опять потерялось.
...
"
  ...
  Вывод команд пишется в лог и одновременно
  выводится в терминал ниже — так видно,
  что реально происходит на каждом шаге:
  ${LOG_FILE}

  Пожалуйста, не закрывай терминал." 12
    sleep 2

    # Вывод дублируем и в лог, и на экран через tee — раньше `exec
    # >>"$LOG_FILE" 2>&1` полностью прятал вывод apt/docker/certbot
    # в файл, и с терминала было не видно, что установка вообще
    # что-то делает (единственная обратная связь — whiptail-баннер
    # show_step между шагами). Диалоги whiptail всё так же пишут
    # напрямую в /dev/tty и не зависят от этого редиректа.
